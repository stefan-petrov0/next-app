...
interface SwitchProps {
  ...
  questionId: string;
  onChange: (questionId: string, newRequired: boolean) => void;
}

export default function Switch({ is_it_enabled, questionId, onChange }:SwitchProps) {
  const [enabled, setEnabled] = useState(is_it_enabled);
  ...
  const handleSwitchChange = async () => {
    setEnabled((prevEnabled) => {
      ...
      onChange(questionId, newEnabledState);
    ...
    <div>
      <label htmlFor="toggle3" className="flex cursor-pointer select-none items-center">
        <div className="relative">
          <input type="checkbox" id="toggle3" className="sr-only" onChange={handleSwitchChange}/>
